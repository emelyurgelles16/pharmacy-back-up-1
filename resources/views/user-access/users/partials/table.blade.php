...
<style>
    ...
    .perm-group .group-header:hover { background: #f1f3f5; }
    .perm-group .group-header .group-title {
        font-weight: 600;
        font-size: 14px;
        color: #1a1a2e;
    }
    .perm-group .group-header .group-title i { color: #0b7a33; margin-right: 8px; }
    ...
    .perm-checkbox:hover { background: #f8f9fa; }
    ...
    .select-all-btn:hover { background: #c8e6c9; transform: scale(1.02); }
    .select-all-btn.deselect { background: #ffebee; border-color: #ef9a9a; color: #c62828; }
    ...
    @media (max-width: 992px) {
        .perm-group .group-body { grid-template-columns: 1fr 1fr; }
    }
    @media (max-width: 768px) {
        ...
        .perm-group .group-body { grid-template-columns: 1fr; }
    }
</style>
                    ...
                    <tbody id="rolesTableBody">
                        ...
                        @php
                            ...
                            $permissionGroups = [
                                ...
                                '🏷️ Promos' => ['view promos', 'manage promos', 'view discounts', 'manage discounts'],
                        ...
                        <tr>
                            ...
                            <td style="vertical-align: top; padding-top: 15px;">
                                <span class="role-badge {{ strtolower(str_replace(' ', '.', $role->name)) }}">
                                    <i class="fas fa-{{ $roleIcon }}"></i> {{ $role->name }}
                                </span>
                                <br>
...
$(document).ready(function() {
    ...
    // ========== ADD ROLE WITH PERMISSIONS (SAME AS EDIT) ==========
    $('#addRoleBtn').click(function() {
        ...
        // Load permissions with empty selection (walang na-check)
        loadPermissions([]);
    });
...
// ========== LOAD PERMISSIONS WITH SELECT ALL ==========
function loadPermissions(selected) {
    ...
    .then(data => {
        ...
        const groups = {
            '📊 Dashboard': ['view dashboard', 'view sales metrics', 'view inventory metrics'],
            '📦 Inventory': ['view inventory', 'view inventory read only', 'create inventory', 'edit inventory', 'update stock', 'delete product', 'edit product', 'view inventory reports'],
            '🛒 POS & Transactions': ['view pos', 'process payment', 'void transaction', 'print receipts', 'view receipts', 'view own receipts'],
            '🏷️ Promos & Discounts': ['view promos', 'manage promos', 'view discounts', 'manage discounts'],
            '📁 Categories': ['view categories', 'manage categories'],
            '💊 Prescriptions': ['view prescriptions'],
            '📈 Reports': ['view sales reports', 'view inventory reports', 'view sales metrics', 'view inventory metrics', 'export reports'],
            '👥 Users & Roles': ['view users', 'create users', 'edit users', 'delete users', 'reset user passwords', 'view roles', 'create roles', 'edit roles', 'delete roles', 'manage roles', 'view permissions', 'create permissions', 'edit permissions', 'delete permissions', 'manage permissions'],
            '⚙️ Settings': ['view settings', 'edit settings'],
            '📜 Logs': ['view logs', 'delete logs'],
            '🧪 Others': ['test permission', 'view own receipts']
        };

        let html = '';
        const permMap = {};
        data.permissions.forEach(p => { permMap[p.name] = p.id; });

        Object.keys(groups).forEach(group => {
            const groupPerms = groups[group];
            const groupPermIds = groupPerms
                .filter(name => permMap[name])
                .map(name => permMap[name]);
            
            if (groupPermIds.length === 0) return;

            const allChecked = groupPermIds.every(id => selected && selected.includes(id));

            html += `
                <div class="perm-group">
                    <div class="group-header">
                        <span class="group-title"><i class="fas fa-folder-open"></i> ${group}</span>
                        <button class="select-all-btn ${allChecked ? 'deselect' : ''}" 
                                data-group="${group.replace(/\s/g, '_')}" 
                                onclick="toggleGroup(this, ${JSON.stringify(groupPermIds)})">
                            ${allChecked ? 'Deselect All' : 'Select All'}
                        </button>
                    </div>
                    <div class="group-body" id="group_${group.replace(/\s/g, '_')}">
            `;

            groupPerms.forEach(permName => {
                const permId = permMap[permName];
                if (!permId) return;
                const checked = selected && selected.includes(permId) ? 'checked' : '';
                html += `
                    <div class="perm-checkbox">
                        <input type="checkbox" class="perm-checkbox" value="${permId}" id="perm_${permId}" ${checked} 
                               onchange="updateSelectAll('${group.replace(/\s/g, '_')}', ${JSON.stringify(groupPermIds)})">
                        <label for="perm_${permId}">${permName}</label>
                    </div>
                `;
            });
        ...
        const ungrouped = data.permissions.filter(p => !groupedNames.includes(p.name));
        if (ungrouped.length > 0) {
            const ungroupedIds = ungrouped.map(p => p.id);
            const allChecked = ungroupedIds.every(id => selected && selected.includes(id));
            
            html += `
                <div class="perm-group">
                    <div class="group-header">
                        <span class="group-title"><i class="fas fa-ellipsis-h"></i> Other Permissions</span>
                        <button class="select-all-btn ${allChecked ? 'deselect' : ''}" 
                                data-group="other_permissions" 
                                onclick="toggleGroup(this, ${JSON.stringify(ungroupedIds)})">
                            ${allChecked ? 'Deselect All' : 'Select All'}
                        </button>
                    </div>
                    <div class="group-body" id="group_other_permissions">
            `;
            ungrouped.forEach(perm => {
                ...
                html += `
                    <div class="perm-checkbox">
                        <input type="checkbox" class="perm-checkbox" value="${perm.id}" id="perm_${perm.id}" ${checked}
                               onchange="updateSelectAll('other_permissions', ${JSON.stringify(ungroupedIds)})">
...
function toggleGroup(btn, permIds) {
    const groupId = $(btn).data('group');
    const checkboxes = $(`#group_${groupId} .perm-checkbox`);
    const isDeselect = $(btn).hasClass('deselect');
    
    checkboxes.prop('checked', isDeselect ? false : true);
    $(btn).text(isDeselect ? 'Select All' : 'Deselect All');
    $(btn).toggleClass('deselect');
    
    // Trigger change event para sa mga checkbox
    checkboxes.trigger('change');
}

// ========== UPDATE SELECT ALL BUTTON ==========
function updateSelectAll(groupId, permIds) {
    const checkboxes = $(`#group_${groupId} .perm-checkbox`);
    const checkedCount = checkboxes.filter(':checked').length;
    const totalCount = checkboxes.length;
    const btn = $(`button[data-group="${groupId}"]`);
    
    if (checkedCount === totalCount && totalCount > 0) {
        btn.text('Deselect All');
        btn.addClass('deselect');
    } else {
        btn.text('Select All');
        btn.removeClass('deselect');
    }
}
