    ...
    <div class="prescription-modal">
        ...
        <div class="modal-body-custom">
            ...
            <div class="info-box">
                ...
                <strong>RX #:</strong> {{ $prescription->prescription_number }} |
                <strong>Created:</strong> {{ $prescription->created_at->format('M d, Y h:i A') }}
            </div>

            <form id="prescriptionForm" action="{{ route('prescriptions.update', $prescription->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-card">
                    ...
                    <div class="form-card-body">
                        <textarea name="special_instructions" class="form-control" rows="2">{{ old('special_instructions', $prescription->special_instructions) }}</textarea>
                    </div>
                </div>
                ...
                <div class="form-card">
                    ...
                    <div class="form-card-body" style="background: #f8f9fa;">
                        <div id="itemsContainer">
                            ...
                            <div class="medication-item" data-item-index="{{ $index }}">
                                <div class="item-header">
                                    <span class="item-number"><i class="fas fa-pills"></i> Item #{{ $index + 1 }}</span>
                                ...
                                <div style="margin-bottom: 12px;">
                                    ...
                                    <div class="searchable-select" data-index="{{ $index }}">
                                        <input type="text" 
                                               ...
                                               autocomplete="off"
                                               value="{{ $item->product ? $item->product->name : $item->product_name }}{{ $item->product && $item->product->brand ? ' ('.$item->product->brand.')' : '' }}"
                                               data-index="{{ $index }}">
                                        <span class="searchable-select-arrow">▼</span>
                                        <div class="searchable-select-list" data-list="{{ $index }}">
                                            @foreach($products as $product)
                                            <div class="searchable-select-option {{ $item->product_id == $product->id ? 'selected' : '' }}" 
                                        ...
                                        <input type="hidden" name="items[{{ $index }}][product_id]" class="medicine-value" value="{{ $item->product_id }}">
                                    </div>
                                </div>

                                <div class="auto-info-box {{ $item->product ? 'show' : '' }}" id="info-{{ $index }}">
                                    <div class="info-row">
                                        <span class="info-label">Type:</span>
                                        <span class="info-value" id="type-{{ $index }}">{{ $item->product ? $item->product->type : '—' }}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Brand:</span>
                                        <span class="info-value" id="brand-{{ $index }}">{{ $item->product ? $item->product->brand : '—' }}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Dosage:</span>
                                        <span class="info-value" id="dosage-{{ $index }}">{{ $item->product ? $item->product->dosage_amount . ' ' . $item->product->dosage_unit : '—' }}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Stock Available:</span>
                                        <span class="info-value" id="stock-{{ $index }}">{{ $item->product ? ($item->product->total_pieces_left ?? 0) . ' pcs' : '—' }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="form-row-3" style="margin-top: 12px;">
                                    <div>
                                        ...
                                        <input type="number" name="items[{{ $index }}][quantity]" class="form-control quantity-input" 
                                               value="{{ $item->quantity_prescribed }}" required min="1" data-index="{{ $index }}">
                                        <small class="stock-warning" id="warning-{{ $index }}" style="color: #dc3545; font-size: 11px; display: none;"></small>
                                    </div>
                                    <div>
                                        <label class="form-label">Frequency</label>
                                        <input type="text" name="items[{{ $index }}][frequency]" class="form-control" 
                                               value="{{ $item->frequency }}" placeholder="e.g., 3x/day">
                                    </div>
                                    <div>
                                        <label class="form-label">Duration</label>
                                        <input type="text" name="items[{{ $index }}][duration]" class="form-control" 
                                               value="{{ $item->duration }}" placeholder="e.g., 7 days">
                                    </div>
                                </div>
                                ...
                                <div style="margin-top: 12px;">
                                    ...
                                    <input type="text" name="items[{{ $index }}][special_note]" class="form-control" 
                                           value="{{ $item->special_note }}" placeholder="e.g., Take before bedtime, With meals, etc.">
                                </div>
                            </div>
                            @endforeach
                        </div>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success || response.status === 'success') {
                        ...
                        html: `
                            <div style="text-align: left; padding: 10px;">
                                <p style="font-size: 15px; margin-bottom: 8px;">
                                    <strong>RX #:</strong> ${response.prescription_number || '{{ $prescription->prescription_number }}'}
                                </p>
                                <p style="font-size: 14px; color: #666; margin-bottom: 8px;">
                                    <strong>Patient:</strong> ${response.patient_name || '{{ $prescription->patient_name }}'}
                                </p>
                    ...
                    submitBtn.prop('disabled', false).html('<i class="fas fa-save"></i> Update Prescription');
                }
            },
            error: function(xhr) {
